...
def driveXDistance(setpoint,duration):
    ...
    while(brain.timer_time(SECONDS)<duration):
        ...
        if (location.position(X, MM)< setpoint - tolerance):
            drivetrain.drive(FORWARD)
        elif (location.position(X, MM)> setpoint + tolerance):
...
def driveYDistance(setpoint,duration):
    ...
    while(brain.timer_time(SECONDS)<duration):
        ...
        if (location.position(Y, MM)< setpoint - tolerance):
            drivetrain.drive(FORWARD)
        elif (location.position(Y, MM)> setpoint + tolerance):
            drivetrain.drive(REVERSE)
        else :
            drivetrain.stop()
...
def driveUsingDistanceSensor(setpoint,duration):
    ...
    while(brain.timer_time(SECONDS)<duration):
        ...
        if (distance.get_distance(MM)> setpoint):
            drivetrain.drive(FORWARD)
        else :
            drivetrain.stop()
